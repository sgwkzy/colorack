...
function backupReadDb({ kitPhotos = [], kitWishlist = [], kitWishlistColors = [], kitWishlistColorPaints = [], kitWishlistPhotos = [] } = {}) {
  return {
    async withExclusiveTransactionAsync(fn) { await fn(this); },
    async getAllAsync(sql) {
      if (sql.includes('FROM kit_photos')) return kitPhotos;
      if (sql.includes('FROM kit_wishlist_photos')) return kitWishlistPhotos;
      if (sql.includes('FROM kit_wishlist_color_paints')) return kitWishlistColorPaints;
      if (sql.includes('FROM kit_wishlist_colors')) return kitWishlistColors;
...
test('v7 snapshot excludes orphaned candidate-photo rows left by older resets', async () => {
  let photoSql = '';
  const db = {
    async withExclusiveTransactionAsync(fn) { await fn(this); },
    async getAllAsync(sql) {
...
test('v7 snapshot stores independent kit shopping candidates and photos', async () => {
...
test('v7 photo backups include empty owned and candidate photo arrays', async () => {
...
test('Light authority device includes current synced photo metadata', async () => {
  const backup = loadCloudBackup({
    db: backupReadDb({
      ...
      kitWishlist: [{ id: 2, name: 'Candidate', maker: 'Maker', series: null, category: null, scale: null, note: null, price: null, added_at: null }],
...
test('Light authority device writes empty photo metadata after local deletion', async () => {
  const backup = loadCloudBackup({
    db: backupReadDb(),
    appOwnership: 'standalone',
    getEntitlements: () => ({ hasBackup: true, hasPhotoBackup: false }),
...
test('Light non-authority device omits photo metadata', async () => {
  const backup = loadCloudBackup({
    db: backupReadDb({
      kitPhotos: [{ kit_id: 1, storage_path: 'users/u/kit-photos/owned.jpg', sort_order: 0 }],
      kitWishlistPhotos: [{ wishlist_id: 2, storage_path: 'users/u/kit-photos/candidate.jpg', sort_order: 1 }],
  ...
  assert.equal(snapshot.kitPhotos, undefined);
  assert.equal(snapshot.kitWishlistPhotos, undefined);
});
...
test('v7 snapshot stores candidate colors and paints by local reference and catalog code', async () => {
  const db = backupReadDb({
    kitWishlist: [
      { id: 1, name: 'RX-78', maker: 'Bandai', series: 'HG', category: null, scale: '1/144', note: null, price: 2200, added_at: '2026-08-31' },
      { id: 2, name: 'Zaku II', maker: 'Bandai', series: 'HG', category: null, scale: '1/144', note: 'green', price: 1800, added_at: '2026-08-30' },
    ],
    kitWishlistColors: [
      { id: 10, wishlist_id: 1, name: 'Armor blue', note: null, sort_order: 0, added_at: '2026-08-31T10:00:00Z' },
      { id: 20, wishlist_id: 2, name: null, note: 'base mix', sort_order: 1, added_at: null },
    ],
    kitWishlistColorPaints: [
      { id: 100, wishlist_color_id: 10, catalog_code: 'brand|series|blue', brand: 'brand', series: 'series', code: 'blue', ratio: 0.25, sort_order: 0 },
      { id: 102, wishlist_color_id: 20, catalog_code: 'brand|series|red', brand: 'brand', series: 'series', code: 'red', ratio: 1, sort_order: 0 },
      { id: 101, wishlist_color_id: 10, catalog_code: 'brand|series|white', brand: 'brand', series: 'series', code: 'white', ratio: 0.75, sort_order: 1 },
    ],
  });
  const backup = loadCloudBackup({ db, appOwnership: 'standalone', downloadKitPhotosForRestore: async () => new Map(), deleteKitPhoto: async () => {} });
  ...
  assert.equal(snapshot.schemaVersion, 7);
  assert.deepEqual(snapshot.kitWishlistColors, [
    { localRef: 'kitwishlistcolor_10', wishlistLocalRef: 'kit_wishlist_1', name: 'Armor blue', note: null, sort_order: 0, added_at: '2026-08-31T10:00:00Z' },
    { localRef: 'kitwishlistcolor_20', wishlistLocalRef: 'kit_wishlist_2', name: null, note: 'base mix', sort_order: 1, added_at: null },
  ]);
  assert.deepEqual(snapshot.kitWishlistColorPaints, [
    { wishlistColorLocalRef: 'kitwishlistcolor_10', catalogCode: 'brand|series|blue', ratio: 0.25, sort_order: 0 },
    { wishlistColorLocalRef: 'kitwishlistcolor_20', catalogCode: 'brand|series|red', ratio: 1, sort_order: 0 },
    { wishlistColorLocalRef: 'kitwishlistcolor_10', catalogCode: 'brand|series|white', ratio: 0.75, sort_order: 1 },
  ]);
});

test('v7 snapshot round-trips two candidates with single and mixed used colors', async () => {
  const statements = [];
  let nextWishlistId = 41;
  let nextColorId = 71;
  const catalogIds = new Map([
    ['brand|series|blue', 501],
    ['brand|series|red', 502],
    ['brand|series|white', 503],
  ]);
  const db = backupReadDb({
    kitWishlist: [
      { id: 1, name: 'RX-78', maker: 'Bandai', series: 'HG', category: null, scale: '1/144', note: null, price: 2200, added_at: '2026-08-31' },
      { id: 2, name: 'Zaku II', maker: 'Bandai', series: 'HG', category: null, scale: '1/144', note: null, price: 1800, added_at: '2026-08-30' },
    ],
    kitWishlistColors: [
      { id: 10, wishlist_id: 1, name: 'Armor blue', note: null, sort_order: 3, added_at: '2026-08-31T10:00:00Z' },
      { id: 20, wishlist_id: 2, name: 'Base mix', note: 'shadow', sort_order: 8, added_at: '2026-08-31T11:00:00Z' },
    ],
    kitWishlistColorPaints: [
      { wishlist_color_id: 10, catalog_code: 'brand|series|blue', brand: 'brand', series: 'series', code: 'blue', ratio: 1, sort_order: 0 },
      { wishlist_color_id: 20, catalog_code: 'brand|series|red', brand: 'brand', series: 'series', code: 'red', ratio: 0.25, sort_order: 0 },
      { wishlist_color_id: 20, catalog_code: 'brand|series|white', brand: 'brand', series: 'series', code: 'white', ratio: 0.75, sort_order: 1 },
    ],
  });
  db.getFirstAsync = async (sql, params) => {
    if (!sql.startsWith('SELECT id FROM catalog_paints')) return null;
    const id = catalogIds.get(params[0]);
    return id ? { id } : null;
  };
  db.runAsync = async (sql, args) => {
    statements.push([sql, args]);
    if (sql.startsWith('INSERT INTO kit_wishlist (')) return { lastInsertRowId: nextWishlistId++ };
  ...
  const snapshot = await backup.buildBackupSnapshot();
  await backup.restoreFromSnapshot(snapshot, 'user-1');

  assert.deepEqual(snapshot.kitWishlist.map(({ localRef }) => localRef), ['kit_wishlist_1', 'kit_wishlist_2']);
  assert.deepEqual(snapshot.kitWishlistColors, [
    { localRef: 'kitwishlistcolor_10', wishlistLocalRef: 'kit_wishlist_1', name: 'Armor blue', note: null, sort_order: 3, added_at: '2026-08-31T10:00:00Z' },
    { localRef: 'kitwishlistcolor_20', wishlistLocalRef: 'kit_wishlist_2', name: 'Base mix', note: 'shadow', sort_order: 8, added_at: '2026-08-31T11:00:00Z' },
  ]);
  assert.deepEqual(snapshot.kitWishlistColorPaints, [
    { wishlistColorLocalRef: 'kitwishlistcolor_10', catalogCode: 'brand|series|blue', ratio: 1, sort_order: 0 },
    { wishlistColorLocalRef: 'kitwishlistcolor_20', catalogCode: 'brand|series|red', ratio: 0.25, sort_order: 0 },
    { wishlistColorLocalRef: 'kitwishlistcolor_20', catalogCode: 'brand|series|white', ratio: 0.75, sort_order: 1 },
  ]);
  assert.deepEqual(statements.filter(([sql]) => sql.startsWith('INSERT INTO kit_wishlist_colors')).map(([, args]) => args), [
    [41, 'Armor blue', null, 3, '2026-08-31T10:00:00Z'],
    [42, 'Base mix', 'shadow', 8, '2026-08-31T11:00:00Z'],
  ]);
  assert.deepEqual(statements.filter(([sql]) => sql.startsWith('INSERT INTO kit_wishlist_color_paints')).map(([, args]) => args), [
    [71, 501, 1, 0],
    [72, 502, 0.25, 0],
    [72, 503, 0.75, 1],
  ]);
...
test('v7 restore inserts candidate colors and skips orphan parents and catalog paints', async () => {
  const statements = [];
  const db = {
    async withExclusiveTransactionAsync(fn) { await fn(this); },
    async getAllAsync() { return []; },
    async getFirstAsync(_sql, params) {
      return params?.[0] === 'brand|series|known' ? { id: 8 } : null;
    },
    async runAsync(sql, args) {
      statements.push([sql, args]);
      if (sql.startsWith('INSERT INTO kit_wishlist (')) return { lastInsertRowId: 41 };
      if (sql.startsWith('INSERT INTO kit_wishlist_colors')) return { lastInsertRowId: 71 };
      return { lastInsertRowId: 0 };
    },
  };
  const backup = loadCloudBackup({ db, appOwnership: 'standalone', downloadKitPhotosForRestore: async () => new Map(), deleteKitPhoto: async () => {} });
  ...
  await backup.restoreFromSnapshot({
    ...
    kitWishlist: [{ localRef: 'kit_wishlist_1', name: 'RX-78', maker: 'Bandai', series: null, category: null, scale: null, note: null, price: null, added_at: '2026-08-31' }],
    kitWishlistColors: [
      { localRef: 'kitwishlistcolor_10', wishlistLocalRef: 'kit_wishlist_1', name: 'Armor blue', note: null, sort_order: 2, added_at: '2026-08-31T10:00:00Z' },
      { localRef: 'kitwishlistcolor_missing', wishlistLocalRef: 'missing-candidate', name: 'Orphan', note: null, sort_order: 3, added_at: null },
    ],
    kitWishlistColorPaints: [
      { wishlistColorLocalRef: 'kitwishlistcolor_10', catalogCode: 'brand|series|known', ratio: 0.25, sort_order: 0 },
      { wishlistColorLocalRef: 'kitwishlistcolor_10', catalogCode: 'brand|series|missing', ratio: 0.75, sort_order: 1 },
      { wishlistColorLocalRef: 'kitwishlistcolor_missing', catalogCode: 'brand|series|known', ratio: 1, sort_order: 0 },
    ],
  }, 'user-1');

  const candidateInsertIndex = statements.findIndex(([sql]) => sql.startsWith('INSERT INTO kit_wishlist ('));
  const colorInsertIndex = statements.findIndex(([sql]) => sql.startsWith('INSERT INTO kit_wishlist_colors'));
  const colorPaintInserts = statements.filter(([sql]) => sql.startsWith('INSERT INTO kit_wishlist_color_paints'));
  assert.ok(candidateInsertIndex >= 0 && candidateInsertIndex < colorInsertIndex);
  assert.equal(statements.filter(([sql]) => sql.startsWith('INSERT INTO kit_wishlist_colors')).length, 1);
  assert.deepEqual(statements[colorInsertIndex][1], [41, 'Armor blue', null, 2, '2026-08-31T10:00:00Z']);
  assert.deepEqual(colorPaintInserts, [[
    'INSERT INTO kit_wishlist_color_paints (wishlist_color_id, paint_id, ratio, sort_order) VALUES (?, ?, ?, ?)',
    [71, 8, 0.25, 0],
  ]]);
});

test('v6 restore leaves candidate used colors empty when color fields are absent', async () => {
  const statements = [];
  const backup = loadCloudBackup({
  ...
  assert.equal(statements.filter(([sql]) => sql.startsWith('INSERT INTO kit_wishlist ')).length, 1);
  assert.equal(statements.filter(([sql]) => sql.startsWith('INSERT INTO kit_wishlist_colors')).length, 0);
  assert.equal(statements.filter(([sql]) => sql.startsWith('INSERT INTO kit_wishlist_color_paints')).length, 0);
});

test('restore deletes candidate color paints and colors before candidate photos and candidates', async () => {
  const sqlCalls = [];
  const db = {
    async withExclusiveTransactionAsync(fn) { await fn(this); },
    async getAllAsync() { return []; },
  ...
  await backup.restoreFromSnapshot(emptySnapshot(7), 'user-1');

  const indexOf = (sql) => sqlCalls.indexOf(sql);
  const paintDelete = indexOf('DELETE FROM kit_wishlist_color_paints');
  const colorDelete = indexOf('DELETE FROM kit_wishlist_colors');
  const photoDelete = indexOf('DELETE FROM kit_wishlist_photos');
  const candidateDelete = indexOf('DELETE FROM kit_wishlist');
  assert.ok(paintDelete >= 0 && paintDelete < colorDelete);
  assert.ok(colorDelete < photoDelete && photoDelete < candidateDelete);
});
...
test('restore removes stale Box references from completed kits', async () => {
  let insertedKitArgs;
  const db = {
    async withExclusiveTransactionAsync(fn) { await fn(this); },
    async getAllAsync() { return []; },
    async runAsync(sql, args) {
      if (sql.startsWith('INSERT INTO kit_boxes')) return { lastInsertRowId: 4 };
      if (sql.startsWith('INSERT INTO kits')) {
        insertedKitArgs = args;
  ...
  const snapshot = {
    ...
    kits: [{
      localRef: 'kit_1', kitBoxLocalRef: 'kitbox_1', name: 'Completed', maker: 'Maker',
      series: null, category: null, scale: null, note: null, price: null,
      status: 'completed', added_at: null, status_changed_at: null,
    }],
  };

  await cloudBackup.restoreFromSnapshot(snapshot, 'user-1');

  assert.equal(insertedKitArgs[0], null);
  assert.equal(insertedKitArgs[8], 'completed');
});
...
test('successful Standard restore records the photo metadata authority UID', async () => {
  const settings = new Map([
    ['cloud_backup_ready_uid', 'user-1'],
    ['cloud_backup_photo_metadata_authority_uid', 'old-user'],
  ]);
  const backup = loadCloudBackup({
    db: restoreDb([]),
    appOwnership: 'standalone',
  ...
  assert.equal(settings.get('cloud_backup_photo_metadata_authority_uid'), 'user-1');
});

test('successful Light restore clears the photo metadata authority UID', async () => {
  const settings = new Map([
    ['cloud_backup_ready_uid', 'user-1'],
    ['cloud_backup_photo_metadata_authority_uid', 'user-1'],
  ]);
  const backup = loadCloudBackup({
    db: restoreDb([]),
    appOwnership: 'standalone',
  ...
  assert.equal(settings.get('cloud_backup_photo_metadata_authority_uid'), '');
});

test('failed restore leaves the photo metadata authority UID unchanged', async () => {
  const settings = new Map([
    ['cloud_backup_ready_uid', 'user-1'],
    ['cloud_backup_photo_metadata_authority_uid', 'user-1'],
  ]);
  ...
  await assert.rejects(backup.restoreFromSnapshot({
    ...emptySnapshot(7),
    kits: [{
      localRef: 'kit_1',
      kitBoxLocalRef: null,
    ...
    kitPhotos: [{ kitLocalRef: 'kit_1', storagePath: 'users/u/kit-photos/photo.jpg', sort_order: 0 }],
  }, 'user-1'), /download failed/);
  assert.equal(settingCalls.filter(([key]) => key === 'cloud_backup_photo_metadata_authority_uid').length, 0);
  assert.equal(settings.get('cloud_backup_photo_metadata_authority_uid'), 'user-1');
});

test('restore rejects when the Firebase user changes during orphan photo cleanup', async () => {
  ...
  const backup = loadCloudBackup({
    db: {
      async withExclusiveTransactionAsync(fn) { await fn(this); },
      async getAllAsync(sql) {
        if (sql === 'SELECT uri FROM kit_photos') return [{ uri: 'file:///orphan.jpg' }];
        return [];
      },
      async runAsync() { return { lastInsertRowId: 0 }; },
    },
    appOwnership: 'standalone',
  ...
  const restore = backup.restoreFromSnapshot(emptySnapshot(7), 'user-1');
...
test('successful Standard push records the photo metadata authority UID after Firestore succeeds', async () => {
  const events = [];
  const settings = new Map([
    ['cloud_backup_ready_uid', 'user-1'],
  ]);
  ...
  await backup.pushBackupToFirestore();

  const firestoreIndex = events.findIndex(([type]) => type === 'firestore');
  const authorityIndex = events.findIndex(([, key]) => key === 'cloud_backup_photo_metadata_authority_uid');
  assert.ok(firestoreIndex >= 0);
  assert.ok(authorityIndex > firestoreIndex);
  assert.equal(settings.get('cloud_backup_photo_metadata_authority_uid'), 'user-1');
});

test('failed Firestore push does not change the photo metadata authority UID', async () => {
  const settings = new Map([
    ['cloud_backup_ready_uid', 'user-1'],
    ['cloud_backup_photo_metadata_authority_uid', 'user-1'],
  ]);
  ...
  await assert.rejects(backup.pushBackupToFirestore(), /firestore failed/);
  assert.equal(settingCalls.filter(([key]) => key === 'cloud_backup_photo_metadata_authority_uid').length, 0);
  assert.equal(settings.get('cloud_backup_photo_metadata_authority_uid'), 'user-1');
});
