...
test('v6 restore rejects a candidate photo with no matching candidate', async () => {
  let transactionCalls = 0;
  const backup = loadCloudBackup({
    db: {
      async withExclusiveTransactionAsync() { transactionCalls++; },
    },
    appOwnership: 'standalone',
    downloadKitPhotosForRestore: async () => new Map(),
    deleteKitPhoto: async () => {},
  ...
  await assert.rejects(
    backup.restoreFromSnapshot({
      ...emptySnapshot(6),
      kitWishlistPhotos: [{ wishlistLocalRef: 'missing', storagePath: 'users/u/kit-photos/candidate.jpg', sort_order: 0 }],
    }, 'user-1'),
    /candidate/i,
  );
