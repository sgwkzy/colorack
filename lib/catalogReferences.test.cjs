...
function loadSeedCatalog(db, seed) {
  const source = fs.readFileSync(require.resolve('./db/seedCatalog.ts'), 'utf8');
  const code = ts.transpileModule(source, {
    compilerOptions: {
      esModuleInterop: true,
      module: ts.ModuleKind.CommonJS,
  ...
  new Function('require', 'module', 'exports', code)(
    (id) => {
      ...
      if (id === '../../assets/seed_catalog.json') return seed;
      if (id === './types') return { catalogCode: (brand, series, code) => `${brand}|${series}|${code}` };
      throw new Error(`Unexpected require: ${id}`);
    },
    module,
    module.exports
...
test('deleting a referenced manual paint throws before destructive SQL', async () => {
  ...
  await assert.rejects(catalog.deletePaint(4), (error) => {
    assert.equal(error.name, 'PaintReferencedByColorError');
    assert.match(error.message, /candidate color/);
    return true;
  });
...
test('reference error copy identifies purchase candidate colors', () => {
  const catalog = loadCatalog({});
  const error = new catalog.PaintReferencedByColorError();
  const en = JSON.parse(fs.readFileSync(require.resolve('../translations/en.json'), 'utf8'));
  const ja = JSON.parse(fs.readFileSync(require.resolve('../translations/ja.json'), 'utf8'));

  assert.match(error.message, /candidate color/);
  assert.match(en.paintReferencedByColor, /purchase candidate color/);
  assert.match(ja.paintReferencedByColor, /購入候補の色/);
});
...
test('seed catalog cleanup preserves candidate color paint references', async () => {
  const statements = [];
  const db = {
    async withTransactionAsync(fn) {
      await fn();
  ...
  const seedCatalog = loadSeedCatalog(db, [{
    brand: 'Brand', series: 'Series', series_en: null, code: 'new', name_ja: 'New', name_en: null,
    hex: '#123456', rgb_r: 18, rgb_g: 52, rgb_b: 86, lab_l: 20, lab_a: 0, lab_b: 0,
    barcode: null, gloss: null, paint_type: null,
  }]);

  await seedCatalog.upsertCatalogFromSeed(db);

  const cleanup = statements.find(([sql]) => sql.startsWith("DELETE FROM catalog_paints WHERE source = 'catalog'"));
  assert.ok(cleanup);
  assert.match(cleanup[0], /id NOT IN \(SELECT paint_id FROM kit_wishlist_color_paints\)/);
});
