...
test('reordering recipes stores zero-based order in one transaction', async () => {
  const statements = [];
  let transactionCalls = 0;
  const db = {
    async withTransactionAsync(fn) {
  ...
  await api.reorderMixRecipes([12, 4, 9]);

  assert.equal(transactionCalls, 1);
  assert.deepEqual(statements, [
    ['UPDATE mix_recipes SET sort_order = ? WHERE id = ?', [0, 12]],
    ['UPDATE mix_recipes SET sort_order = ? WHERE id = ?', [1, 4]],
    ['UPDATE mix_recipes SET sort_order = ? WHERE id = ?', [2, 9]],
  ]);
});

test('adding a recipe appends it after existing recipes', async () => {
  const statements = [];
  let transactionCalls = 0;
  const db = {
    async withTransactionAsync(fn) {
    ...
    async getFirstAsync(sql) {
      statements.push(['SELECT', sql]);
      return { n: 4 };
    },
    async runAsync(sql, args) {
      statements.push(['RUN', sql, args]);
      return { lastInsertRowId: 12 };
    },
  };
  const api = loadMixRecipes(db);

  await api.addMixRecipe('Ocean', 'memo', []);

  assert.equal(transactionCalls, 1);
  assert.deepEqual(statements, [
    ['SELECT', 'SELECT COALESCE(MAX(sort_order), -1) + 1 AS n FROM mix_recipes'],
    ['RUN', 'INSERT INTO mix_recipes (name, note, sort_order) VALUES (?, ?, ?)', ['Ocean', 'memo', 4]],
  ]);
});
