...
function loadAuth({
  ...
  const authInstance = {
    app: { options: { projectId: 'colorack-test' } },
...
test('Apple account deletion reauthenticates and revokes before server deletion', async () => {
  const events = [];
  const auth = loadAuth({
  ...
  const result = await auth.deleteFirebaseAccount(async (request) => {
  ...
  assert.equal(result.appleManualRevocationRequired, false);
});

test('Apple and Google linked account uses Google deletion on Android', async () => {
  const auth = loadAuth({
    platform: 'android',
    initialProviderIds: ['apple.com', 'google.com'],
    linkSubscriptionUser: async () => {},
  ...
  const result = await auth.deleteFirebaseAccount(async () => {
    serverDeleted = true;
  });

  assert.equal(serverDeleted, true);
  assert.equal(result.appleManualRevocationRequired, true);
});

test('Apple-only account deletion is blocked on Android before server changes', async () => {
  const auth = loadAuth({
    platform: 'android',
    initialProviderIds: ['apple.com'],
    linkSubscriptionUser: async () => {},
  ...
  assert.equal(serverDeleted, false);
});
