...
test('purchase candidate detail routes Box move and delete to the existing list actions', () => {
  const detail = fs.readFileSync(require.resolve('../components/KitWishlistDetailModal.tsx'), 'utf8');
  const screen = fs.readFileSync(require.resolve('../app/(tabs)/kit-wishlist.tsx'), 'utf8');

  assert.match(detail, /onMove\(currentItem\)/);
  assert.match(detail, /onDelete\(currentItem\)/);
