...
test('purchase candidate editor routes Box move and delete to the existing list actions', () => {
  ...
  assert.match(editor, /onEditAction\?: \(action: 'move' \| 'delete'\) => void/);
  assert.match(editor, /editingWishlist && onEditAction/);
  assert.match(editor, /t\('moveToBox'\)/);
  assert.match(editor, /t\('delete'\)/);
  assert.match(screen, /onEditAction=\{\(action\) => \{[\s\S]*?requestMove\(item\)[\s\S]*?deleteItem\(item\)/);
});
