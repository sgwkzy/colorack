...
test('kit shopping list supports accessible delete and guards delete and undo failures', () => {
  ...
  assert.match(source, /accessibilityActions=\{\[\{ name: 'delete', label: t\('delete'\) \}\]\}/);
  assert.match(source, /onAccessibilityAction=\{\(\{ nativeEvent \}\) => \{[\s\S]*?actionName === 'delete'[\s\S]*?deleteItem\(item\)/);
