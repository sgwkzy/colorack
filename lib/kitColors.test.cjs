...
test('kit color child routes keep their parent modal mounted and color editing available', () => {
  ...
  const composerModal = composer.slice(composer.indexOf('<Modal visible={visible}'), composer.indexOf('</Modal>'));

  assert.match(detail, /<Modal visible=\{visible\}/);
  assert.match(detail, /<ColorMixDetailModal[\s\S]*?\n\s+editable\n/);
  assert.match(composer, /<Modal visible=\{visible\}/);
  assert.match(composer, /<TouchableOpacity[\s\S]*?onPress=\{\(\) => addSavedMix\(item\)\}/);
  assert.match(detailModal, /<KitColorComposerModal/);
  assert.match(detailModal, /<ColorMixDetailModal/);
  assert.match(detailModal, /<ColorMixEditorModal/);
  assert.match(composerModal, /<ColorMixPaintPickerModal/);
  assert.match(composerModal, /<ColorMixEditorModal/);
});
