...
function readSource(relativePath) {
  return fs.readFileSync(require.resolve(relativePath), 'utf8').replace(/\r\n/g, '\n');
}
...
test('add kit color rejects a missing kit before writing any rows', async () => {
  const statements = [];
  const transactionModes = [];
  const db = {
    async withTransactionAsync(fn) {
      transactionModes.push('regular');
      await fn(this);
    },
    ...
    async getFirstAsync(sql, args) {
      statements.push([sql, args]);
      if (sql === 'SELECT id FROM kits WHERE id = ?') return null;
      return { n: 0 };
    },
    async runAsync(sql, args) {
      statements.push([sql, args]);
  ...
  await assert.rejects(
    () => api.addKitColor(404, 'Missing kit', null, [{ paintId: 4, ratio: 1 }]),
    /Kit not found/
  );

  assert.deepEqual(transactionModes, ['exclusive']);
  assert.equal(statements.some(([sql]) => sql.startsWith('INSERT INTO kit_colors')), false);
  assert.equal(statements.some(([sql]) => sql.startsWith('INSERT INTO kit_color_paints')), false);
});
...
test('kit color composer stays inside the kit modal without adding a native modal layer', () => {
  const detail = readSource('../components/KitDetailModal.tsx');
  const wishlistDetail = readSource('../components/KitWishlistDetailModal.tsx');
  const panel = readSource('../components/KitUsedColorsPanel.tsx');
  const composer = readSource('../components/KitColorComposerModal.tsx');
  const editorModal = readSource('../components/ColorMixEditorModal.tsx');
  const paintPicker = readSource('../components/ColorMixPaintPickerModal.tsx');
  const detailModal = detail.slice(detail.indexOf('<Modal visible={visible}'), detail.indexOf('</Modal>'));
  const panelBody = panel.slice(panel.indexOf('export default function KitUsedColorsPanel'), panel.indexOf('export function KitUsedColorsOverlays'));

  for (const source of [detail, wishlistDetail]) {
    const scrollEnd = source.indexOf('</SwipeDownScrollView>');
    const overlayStart = source.indexOf('<KitUsedColorsOverlays', scrollEnd);
    const safeAreaEnd = source.lastIndexOf('</SafeAreaView>');
    assert.ok(scrollEnd >= 0);
    assert.ok(overlayStart > scrollEnd && overlayStart < safeAreaEnd, 'used-color overlays must be outside the detail scroll view');
  }

  assert.match(detail, /<Modal visible=\{visible\}/);
  assert.match(detail, /useKitUsedColorsController\(\{[\s\S]*?repository: usedColorRepository/);
  ...
  assert.doesNotMatch(panelBody, /<KitColorComposerModal|<ColorMixDetailModal|<ColorMixEditorModal/);
  assert.match(panel, /export function KitUsedColorsOverlays[\s\S]*?<KitColorComposerModal/);
  ...
  assert.match(panel, /export function KitUsedColorsOverlays[\s\S]*?<ColorMixEditorModal[\s\S]*?\n\s+embedded\n/);
  ...
  assert.match(panel, /<ColorMixEditorModal[\s\S]*?visible=\{controller\.editingColor\}[\s\S]*?\n\s+embedded\n[\s\S]*?requestCloseRef=\{requestCloseRef\}/);
});
