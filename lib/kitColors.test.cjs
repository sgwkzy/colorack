...
test('kit color delete and reorder failures stay visible to the user', () => {
  ...
  assert.match(saveColorOrder, /try \{/);
  assert.match(saveColorOrder, /setKitColors\(next\)/);
  assert.match(saveColorOrder, /catch \(error\)[\s\S]*?setKitColors\(previous\)[\s\S]*?Alert\.alert\(t\('error'\), t\('saveFailed'\)\)/);
});

test('shared used-color panel owns the list, overlays, drag settings, and accessibility actions', () => {
  const panel = fs.readFileSync(require.resolve('../components/KitUsedColorsPanel.tsx'), 'utf8');

  assert.match(panel, /export interface UsedColorRepository/);
  assert.match(panel, /load\(\): Promise<KitColorSummary\[\]>/);
  assert.match(panel, /add\(name: string \| null, note: string \| null, paints:/);
  assert.match(panel, /update\(colorId: number, name: string \| null, note: string \| null, paints:/);
  assert.match(panel, /remove\(colorId: number\): Promise<void>/);
  assert.match(panel, /reorder\(colorIds: number\[\]\): Promise<void>/);
  assert.match(panel, /repository\.load\(\)/);
  assert.match(panel, /repository\.add\(/);
  assert.match(panel, /repository\.update\(/);
  assert.match(panel, /repository\.remove\(/);
  assert.match(panel, /repository\.reorder\(/);
  ...
  assert.match(panel, /setKitColors\(next\)/);
  assert.match(panel, /catch \(error\)[\s\S]*?setKitColors\(previous\)[\s\S]*?Alert\.alert\(t\('error'\), t\('saveFailed'\)\)/);
