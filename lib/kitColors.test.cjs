...
test('kit color routes replace their parent modal and keep color editing available', () => {
  ...
  assert.match(detail, /<Modal visible=\{visible && !pickerOpen && !colorDetailOpen && !editingColor\}/);
  assert.match(detail, /<ColorMixDetailModal[\s\S]*?\n\s+editable\n/);
  assert.match(composer, /<Modal visible=\{visible && \(route === 'source' \|\| route === 'saved'\)\}/);
