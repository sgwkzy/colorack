...
test('kit color overlays reserve the device safe area exactly once', () => {
  const detail = fs.readFileSync(require.resolve('../components/KitDetailModal.tsx'), 'utf8');
  const composer = fs.readFileSync(require.resolve('../components/KitColorComposerModal.tsx'), 'utf8');
  ...
  assert.match(editor, /embeddedSafeArea\?: boolean/);
  assert.match(editor, /edges=\{!embedded \|\| embeddedSafeArea \? \['top', 'bottom'\] : \[\]\}/);
  assert.match(detail, /<ColorMixEditorModal[\s\S]*?visible=\{editingColor\}[\s\S]*?\n\s+embedded\n\s+embeddedSafeArea\n/);
  assert.doesNotMatch(composer, /<ColorMixEditorModal[\s\S]*?\n\s+embeddedSafeArea\n/);
});
