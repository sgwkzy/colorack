...
test('restored photo entitlement marks local photos for re-upload', async () => {
  ...
  assert.deepEqual(resetSql, [
    'UPDATE kit_photos SET synced_at = NULL, storage_path = NULL',
    'UPDATE kit_wishlist_photos SET synced_at = NULL, storage_path = NULL',
  ]);
  ...
  const scenarios = [
    ...
    },
    {
      ...
      expectedEvents: [
        ...
        'sql:UPDATE kit_wishlist_photos SET synced_at = NULL, storage_path = NULL',
        'login:user-2',
      ],
      expectedEntitlements: { hasBackup: true, hasPhotoBackup: true },
    },
