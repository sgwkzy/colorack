...
- ドロワーのモード切り替えタップを「プレビュー専用」の一時的な表示状態にする。塗料ボックスを開いた状態でドロワーを開き、Kitrackへ切り替えて(実際にはキットボックスへ移動せず)ドロワーを閉じた場合、次にドロワーを開いた時は実際に表示中の画面(塗料ボックス)に合わせてColorackへ戻るようにする
...
### `NavigationDrawer.tsx`: モード切り替えをプレビュー専用にする

現状、ドロワー最上部のColorack/Kitrack切り替えリンクは`setAppMode(otherMode)`を直接呼んでおり、タップした瞬間に永続化される。これだと「実際にはボックスへ移動せず、切り替えて見ただけ」の操作もドロワーの状態として残ってしまい、次にドロワーを開いた時に実際の画面と食い違う。

`useAppMode()`が返す値(＝実際に表示中の画面から自動同期される「本当の」モード)とは別に、ドロワー自身が持つローカルな`previewMode`状態を導入し、ドロワーが開くたびに本当のモードへ同期し直す。

```tsx
const appMode = useAppMode();
const [previewMode, setPreviewMode] = useState<AppMode>(appMode);
// ドロワーを開くたびに本当のモードへ同期し直す。閉じている間にトグルだけして
// 実際にはボックスへ移動しなかった場合、次に開いた時は元のモードに戻る。
useEffect(() => { if (visible) setPreviewMode(appMode); }, [visible, appMode]);
const mode = previewMode; // 以降のタイトル・ドロワー内容の描画は previewMode を使う
```

タイトル行のリンクの`onPress`は`setAppMode(otherMode)`ではなく`setPreviewMode(otherMode)`を呼ぶように変える(グローバルな永続化はせず、ドロワーの表示だけを切り替える)。実際にボックスをタップして画面遷移すれば、遷移先の`owned.tsx`/`kits.tsx`の`useFocusEffect`が`setAppMode`を呼んで本当のモードを更新する(このファイル自体には変更不要、既存の`go`/`goKits`ハンドラのままでよい)。

## 検証方法

- `npx tsc --noEmit`
- UTF-8 BOMなし確認
- 実機での確認: キットボックスAを開いた状態でアプリを完全終了→再起動→キットボックスAが開いた状態・ドロワーがKitrackモードで起動することを確認。塗料ボックスBを開いた状態で同様に確認。使用済み画面・完成品画面を見ている状態で終了→再起動でそれぞれの画面に戻ることを確認。お気に入り画面を見ている状態で終了した場合は、その前に見ていたボックス画面に戻ることを確認(お気に入り自体は復元対象に含めない)。塗料ボックスを開いた状態でドロワーを開き、Kitrackへ切り替えて(キットボックスへは移動せず)ドロワーを閉じ、再度ドロワーを開いた時にColorackへ戻っていることを確認。
