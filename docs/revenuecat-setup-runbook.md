...
3. エンタイトルメントを2つ作成する:
   ...
   - ライトは`backup`、スタンダードは`backup`と`backup_photos`を付与する。両有料プランは広告を非表示にし、圧縮済みキット写真のStorageバックアップを含むのはスタンダードのみとする。
...
- プラン閲覧、購入、購入復元はログイン任意とする。データベースバックアップ、写真バックアップ、およびクラウド復元の前にはログインが必要であり、クラウド操作時はFirebase UIDとRevenueCat App User IDが一致する状態を確認する。
- TestFlight受け入れ確認:
  1. 新規インストール後、ログインせずにプランを開く。
  2. ライトを購入し、広告が消え、写真バックアップが表示されないことを確認する。
  3. スタンダードを購入または復元し、広告と写真バックアップが有効になることを確認する。
  4. 未ログインの有料ユーザーにはバックアップ案内とサインインボタンが表示され、「今すぐバックアップ」操作はログインするまで表示されないことを確認する。
   5. ログイン後にデータベースバックアップが成功することを確認する。
   6. スタンダードでキット写真の圧縮・Storageバックアップと復元を確認する。
   7. サインアウト後も広告非表示が維持され、クラウド操作だけがログイン要求になることを確認する。
   8. アカウントAでバックアップ後にサインアウトし、アカウントBでログインする。Bのクラウドバックアップが無い場合も、Aの端末データが自動でBへ送られず、明示的な引き継ぎ確認が表示されることを確認する。
...
キット写真を個別に削除した場合も、アプリはローカルDBの参照と端末内ファイルだけを削除し、共有されるFirebase Storage側の実体は即時削除しない。キット/キットボックスの一括削除・全リセット・クラウド復元時のローカルデータ一掃でも、Storage側のクリーンアップを行わない(復元直後に復元元のデータを消してしまう事故を避けるため)。また、スタンダードプランを解約したユーザーの写真も、猶予期間後の自動削除(Cloud Functions、未実装)が無い限りStorageに残り続ける。

これらのStorage孤児オブジェクトは、参照の有無を確認できる「定期的なサーバーサイド整理ジョブ(Cloud Functions)」で回収すべきであり、このリポジトリ(Expo Reactネイティブアプリ)側の実装対象ではない。
